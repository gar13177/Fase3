...
public class DDFA {
    ...
    public DDFA(String regex, String alp){
        ...
        for (int i = 0; i< regex.length(); i++){
            nullable[i] = false;
            if (regex.charAt(i)==(char)200 || regex.charAt(i)==(char)569)
                nullable[i] = true;
            else if(regex.charAt(i)==(char)248){
                boolean temp = false;
                for (int j: dg.adj(i)){
            ...
            }else if(regex.charAt(i)==(char)399){
                boolean temp = true;
                for (int j: dg.adj(i)){
        ...
        for (int i = 0; i<regex.length(); i++){
            if (alp.contains(""+regex.charAt(i))|| regex.charAt(i)=='!'){
                ...
                firstpos[i] = temp;
            }else if(regex.charAt(i)==(char)569){
                Set<Integer> temp = new LinkedHashSet();
                for (int j: dg.adj(i)){
                    if (firstpos[j] !=null)
                        temp.addAll(firstpos[j]);
    ...
    public String valTxt(String txt){
        ...
        if (i != 0){
            ...
            for (int j = 0; j < i-1; j++){
                ...
                if (txt.charAt(j+1)==(char)248 || txt.charAt(j+1) == (char)568 || txt.charAt(j+1) == (char)569 || txt.charAt(j)==(char)741 || txt.charAt(j) == (char)248){
